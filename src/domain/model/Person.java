...
public class Person {
    ...
    public String hash(String password) {
        ...
        crypt.reset();
        byte[] passwordInBytes = new byte[0];
        try {
            passwordInBytes = password.getBytes("UTF-8");
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
        crypt.update(passwordInBytes);
        byte[] passwordDigest = crypt.digest();
        BigInteger digestAsBigInt = new BigInteger(1, passwordDigest);
        return digestAsBigInt.toString(16);
    }
